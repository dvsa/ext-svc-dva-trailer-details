...
import {TrailerDetailService} from "../src/services/TrailerDetailService";
jest.mock("../src/services/TrailerDetailService");
        ...
        context("should correctly handle incoming events", () => {
            it("should call functions with correct event payload", async () => {
                ...
                const trailerDetails = {
                    path: "/dva-trailer-details/C432678",
                    pathParameters: null,
                    resource: "/dva-trailer-details",
                    httpMethod: "GET",
                ...
                TrailerDetailService.prototype.getTrailerDetails = jest.fn().mockImplementation(() => {
                    return Promise.resolve(new HTTPResponse(200, {}));
                });

                const result = await handler(trailerDetails, ctx, clbck);
                ctx.succeed(result);
                expect(result.statusCode).toEqual(200);
                expect(TrailerDetailService.prototype.getTrailerDetails).toHaveBeenCalled();
            });
